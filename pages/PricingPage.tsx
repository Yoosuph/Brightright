  ...
  const featureData = [
    {
      ...
      features: [
        ...
        {
          name: 'Keywords per Brand',
          starter: '10',
          business: '50',
          enterprise: 'Unlimited',
        },
        {
          name: 'Competitors per Brand',
          starter: '3',
          business: '25',
          enterprise: 'Unlimited',
        },
    ...
    <div className="space-y-16 animate-fade-in max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 sm:py-16">
      <div className="text-center">
        ...
        <p className="text-gray-400 mt-4 max-w-2xl mx-auto">
          Choose the plan that's right for you. Cancel or upgrade at any time.
        </p>
        <div className="mt-8">
          <BillingToggle
...
const PricingCard: React.FC<PricingCardProps> = ({
  ...
  return (
    <Card
      className={`flex flex-col h-full ${isPopular ? 'border-brand-purple ring-2 ring-brand-purple' : ''}`}
    >
      {isPopular && (
        <div className="bg-brand-purple text-white text-xs font-bold tracking-widest uppercase py-1 px-3 rounded-full self-start -mt-9 -mr-9 shadow-lg">
          Most Popular
        </div>
      )}
...
const IconCheck: React.FC<{ className?: string }> = ({ className }) => (
  <svg
    ...
    className={`h-5 w-5 text-green-500 mt-0.5 ${className}`}
    fill="none"
    viewBox="0 0 24 24"
    stroke="currentColor"
  >
...
const IconCross: React.FC<{ className?: string }> = ({ className }) => (
  <svg
    xmlns="http://www.w3.org/2000/svg"
    className={`h-5 w-5 text-gray-500 mt-0.5 ${className}`}
    fill="none"
    viewBox="0 0 24 24"
    stroke="currentColor"
  >
