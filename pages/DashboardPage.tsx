  ...
  return (
    <div className="relative" ref={ref}>
      <button
        ...
        className="flex items-center space-x-2 px-3 py-1.5 bg-gray-500/10 rounded-md hover:bg-gray-500/20 transition-colors"
      >
        <IconCalendar />
          ...
          <ul className="p-1">
            {ranges.map(range => (
              <li
                ...
                className="px-3 py-1.5 text-sm rounded-md hover:bg-gray-500/10 cursor-pointer"
              >
                {range}
              </li>
            ))}
      ...
      <div className="p-3 bg-gray-800/80 backdrop-blur-sm border border-white/20 rounded-lg text-white shadow-lg text-sm">
        ...
        {payload.map((pld, index: number) => (
          ...
          >
            ...
            <span className="ml-2">
              {pld.value}
              {pld.dataKey.includes('positive') ||
              pld.dataKey.includes('negative')
                ? '%'
                : ''}
            </span>
          </div>
...
const VisibilityScoreCard: React.FC<{ score: number; change: number }> = ({
  score,
  change,
}) => {
  ...
  return (
    <Card className="flex flex-col items-center justify-center text-center h-full">
      ...
      <div
        className={`flex items-center justify-center text-sm font-semibold mt-2 ${changeColor}`}
      >
        <ChangeIcon />
        <span>
          {change >= 0 ? '+' : ''}
...
const SentimentChartCard: React.FC<{ data: SentimentBreakdown }> = ({
  data,
}) => {
  const chartData = [
...
const SentimentTrendChart: React.FC<{ data: SentimentTrendPoint[] }> = ({
  data,
}) => (
...
const PlatformBreakdownChart: React.FC<{ data: PlatformBreakdown[] }> = ({
  data,
}) => (
  <Card>
    <h3 className="font-semibold text-gray-900 dark:text-white mb-4">
...
const ActionableInsightsCard: React.FC<{
    ...
    <Card className="lg:col-span-3">
      ...
      <div className="space-y-4">
        {insights.map((insight, index) => (
          <div
            key={index}
            className="p-4 bg-gray-500/10 rounded-lg flex items-start gap-4"
          >
...
const MentionsTable: React.FC<{ mentions: DetailedMention[] }> = ({
  mentions,
}) => {
  const SentimentIndicator: React.FC<{ sentiment: string }> = ({
    sentiment,
  }) => {
...
const DashboardPlaceholder: React.FC = () => (
  <div className="space-y-6 animate-pulse">
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
      <Card className="h-48">
        <div className="w-full h-full bg-gray-500/20 rounded-lg"></div>
      </Card>
      <Card className="h-48">
        <div className="w-full h-full bg-gray-500/20 rounded-lg"></div>
      </Card>
...
const DashboardPage: React.FC<DashboardPageProps> = ({
  ...
  const [analysisResult, setAnalysisResult] =
    useState<DashboardAnalysisResult | null>(null);
  ...
  const runAnalysis = useCallback(async () => {
    if (!appData?.brandName || !appData?.keywords) {
      setError(
        'Brand name and keywords are not set. Please configure them in Settings.'
      );
      ...
      return (
        <Card className="text-center py-12">
          <p className="text-gray-500 mb-4">
            Welcome! Click "Refresh Data" to generate your dashboard.
          </p>
