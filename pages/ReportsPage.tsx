      ...
      <div className="p-3 bg-gray-700/80 backdrop-blur-sm border border-white/20 rounded-lg text-white text-sm">
        ...
        {payload.map((pld, index: number) => (
          <div
            ...
            style={{ color: pld.color || pld.stroke }}
          >
            <div
...
const generateTrendData = (
  ...
  const startScore = Math.max(
    10,
    Math.min(90, currentScore + (Math.random() - 0.6) * 40)
  );
  for (let i = 0; i < points; i++) {
    const baseProgress = i / (points - 1);
    const baselineScore =
      startScore + (currentScore - startScore) * baseProgress;
    const fluctuationImpact = Math.sin(baseProgress * Math.PI);
    const fluctuation = (Math.random() - 0.5) * 15 * fluctuationImpact;
    const finalScore = Math.max(0, Math.min(100, baselineScore + fluctuation));
...
const ReportHeader: React.FC<{ report: Report; brandName: string }> = ({
  report,
  brandName,
}) => (
  <div className="mb-6 p-4 bg-gray-500/10 rounded-lg">
    ...
    <div className="flex flex-wrap text-sm text-gray-400 mt-2 gap-x-6 gap-y-1">
      ...
      <span>
        <strong>Generated:</strong> {report.dateGenerated}
      </span>
    </div>
  </div>
);

const KeyMetrics: React.FC<{ analysis: DashboardAnalysisResult }> = ({
  analysis,
}) => {
...
const ActionableInsights: React.FC<{ insights?: ActionableInsight[] }> = ({
  insights,
}) => {
    ...
    <Card className="card-for-print">
      ...
      <div className="space-y-4">
        {insights.map((insight, index) => (
          <div
            key={index}
            className="p-4 bg-gray-500/10 rounded-lg flex items-start gap-4 page-break-inside-avoid"
          >
            ...
            <div className="flex-grow">
              <div className="flex flex-col sm:flex-row justify-between sm:items-center mb-1">
                ...
                <span
                  className={`text-xs font-bold px-2 py-0.5 rounded-full ${PRIORITY_STYLES[insight.priority]}`}
                >
                  {insight.priority}
                </span>
              </div>
...
const MentionsTable: React.FC<{
  mentions: DashboardAnalysisResult['mentions'];
}> = ({ mentions }) => (
  <Card className="card-for-print">
    ...
    <div>
      ...
      <div className="hidden print-only pdf-cover page-break-after">
        ...
        <div className="meta-info">
          ...
          <p>
            <strong>Generated on:</strong> {report.dateGenerated}
          </p>
        </div>
      ...
      <div className="hidden print-only pdf-footer">
        <table>
          <tbody>
            <tr>
              <td>
                Generated on {new Date().toLocaleDateString()} by BrightRank.AI
              </td>
...
const ReportDetailView: React.FC<{ report: Report; brandName: string }> = ({
  report,
  brandName,
}) => {
  return (
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 page-break-before page-break-inside-avoid">
            ...
            <ReportChartCard title="Sentiment Trend">
              <ResponsiveContainer width="100%" height="100%">
                <LineChart
                  data={report.analysis.sentimentTrend}
                  margin={{ top: 5, right: 20, left: -10, bottom: 5 }}
                >
                  <CartesianGrid
                    strokeDasharray="3 3"
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 page-break-before page-break-inside-avoid">
                ...
                >
                  ...
                  >
                    {report.competitorComparison.map((entry, index) => (
                      <Cell
                        key={`cell-${index}`}
                        fill={entry.name === brandName ? '#EC4899' : '#6366F1'}
                      />
                    ))}
                  </Bar>
                </BarChart>
            ...
            <ReportChartCard title="Platform Breakdown">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart
                  data={report.analysis.platformBreakdown}
                  margin={{ top: 5, right: 20, left: -10, bottom: 5 }}
                >
                  <CartesianGrid
                    strokeDasharray="3 3"
          ...
          <div className="relative">
            ...
            >
              {dateRanges.map(range => (
                <option key={range} value={range} className="dark:bg-dark-bg">
                  {range}
                </option>
              ))}
            </select>
  ...
  return (
    <div className="relative w-full" ref={ref}>
      <button
        ...
        className="flex items-center w-full justify-between space-x-2 px-3 py-2 bg-gray-500/10 rounded-md hover:bg-gray-500/20 transition-colors text-left"
      >
          ...
          <ul className="p-1 max-h-60 overflow-y-auto">
            {sortedReports.map(report => (
              <li
                ...
                className="px-3 py-2 text-sm rounded-md hover:bg-gray-500/10 cursor-pointer"
              >
                <p className="font-medium truncate">{report.title}</p>
                <p className="text-xs text-gray-400">
                  {report.dateGenerated} - {report.dateRange}
                </p>
              </li>
            ))}
          </ul>
...
const ReportsPage: React.FC<{ appData: OnboardingData }> = ({ appData }) => {
  ...
  const handleCreateReport = useCallback(
    async ({ title, dateRange }: { title: string; dateRange: string }) => {
      setIsLoading(true);
      setError(null);

      const keywordsArray = appData.keywords
        .split(',')
        .map(k => k.trim())
        .filter(Boolean);
      ...
      if (result) {
        ...
        const competitorScores = await getCompetitorScores(
          appData.brandName,
          keywordsArray,
          competitorNames
        );
        ...
        setError(
          'Failed to generate report from Gemini API. Please try again.'
        );
      }
      setIsLoading(false);
    },
  ...
  const renderContent = () => {
    ...
    return (
      <div className="space-y-6">
        {error && (
          <Card className="text-center py-4 bg-red-500/10 text-red-400">
            {error}
          </Card>
        )}
        <Card>
          ...
          {reports.length > 0 ? (
              ...
              {reports
                .map(report => (
                  <li
                    ...
                    className="flex justify-between items-center p-4 rounded-lg bg-gray-500/10 hover:bg-gray-500/20 cursor-pointer transition-colors"
                  >
                    <div>
                      <p className="font-semibold">{report.title}</p>
                      <p className="text-sm text-gray-400">
                        Generated: {report.dateGenerated} | Range:{' '}
                        {report.dateRange}
                      </p>
                    </div>
                    <IconChevronRight />
            ...
            <div className="text-center py-12">
              ...
              <p className="text-gray-400 mb-6">
                Create your first report to start analyzing trends.
              </p>
              <Button onClick={() => setIsCreating(true)}>
...
const IconShieldCheck = () => (
  ...
  >
    <path
      ...
      strokeLinejoin="round"
      strokeWidth={2}
      d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 20.944a11.955 11.955 0 019-2.606 11.955 11.955 0 019 2.606 12.02 12.02 0 00-2.382-9.016z"
    />
  </svg>
);
const IconSearch = () => (
