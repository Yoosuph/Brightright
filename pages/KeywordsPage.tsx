...
const SentimentIcon: React.FC<{
  sentiment: KeywordMentionDetail['sentiment'];
}> = ({ sentiment }) => {
  ...
  switch (sentiment) {
    ...
    default:
      return (
        <div className={`${baseClasses} bg-gray-500/20 text-gray-400`}>
          <IconQuestion />
        </div>
      );
  }
...
const KeywordsPage: React.FC<KeywordsPageProps> = ({
  brandName,
  initialKeywords,
}) => {
  ...
  const handleAnalyze = async () => {
    ...
    if (result) {
      ...
      setError(
        'Failed to get analysis from Gemini API. Please check the console for more details.'
      );
    }
    setIsLoading(false);
  };
        ...
        <Card>
          ...
          <ul className="space-y-2 max-h-96 overflow-y-auto">
            {keywords.map(keyword => (
              <li
                key={keyword}
                className="flex justify-between items-center bg-gray-500/10 p-2 rounded-md"
              >
                <span>{keyword}</span>
                <button
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      ...
                      <div className="text-center p-4 bg-gray-500/10 rounded-lg flex flex-col justify-center">
                        ...
                        <p
                          ...
                          style={{
                            color:
                              SENTIMENT_COLORS[analysisResult.sentiment] ||
                              '#FFFFFF',
                          }}
                        >
                      ...
                      {analysisResult.mentions.length > 0 ? (
                        <div className="space-y-3 max-h-60 overflow-y-auto pr-2">
                          {analysisResult.mentions.map((mention, i) => (
                            <div
                              key={i}
                              className={`group relative flex items-start gap-3 p-3 bg-black/20 rounded-md border-l-4 ${MENTION_BORDER_COLORS[mention.sentiment]}`}
                            >
                              <div className="flex-shrink-0 pt-0.5">
                                <SentimentIcon sentiment={mention.sentiment} />
                              </div>
                              <p className="min-w-0 text-gray-300 text-sm truncate">
                                {mention.text}
                              </p>
                        ...
                        <p className="text-gray-500 text-center py-2 text-sm">
                          No specific mentions found in the analysis.
                        </p>
                      )}
                    </div>
                  </div>
              ...
              if (!error) {
                return (
                  <div className="text-center text-gray-500 py-10 min-h-[200px] flex items-center justify-center">
                    <p>
                      Click "Analyze" to see your brand's visibility report from
                      Gemini.
                    </p>
                  </div>
                );
...
const IconThumbUp = () => (
  ...
  >
    <path
      ...
      strokeLinejoin="round"
      strokeWidth={2}
      d="M14 10h4.764a2 2 0 011.789 2.894l-3.5 7A2 2 0 0115.263 21h-4.017c-.163 0-.326-.02-.485-.06L7 18.734V11a2 2 0 012-2h2V4a2 2 0 012-2v6z"
    />
  </svg>
);
const IconThumbDown = () => (
  ...
  >
    <path
      ...
      strokeLinejoin="round"
      strokeWidth={2}
      d="M10 14H5.236a2 2 0 01-1.789-2.894l3.5-7A2 2 0 018.738 3h4.017c.163 0 .326.02.485.06L17 5.266V13a2 2 0 01-2 2h-2v6a2 2 0 01-2 2v-6z"
    />
  </svg>
);
const IconMinus = () => (
